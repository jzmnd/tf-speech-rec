...
from models import convSpeechModelA, convSpeechModelB
...
def main(_):
    ...
    # Model parameters (PUT INTO A PARAM FILE)
    ...
    window = 512                                        # Length of audio window
    num_mel_bins = 64
    num_mfccs = 20
    ...
    w = 0.0750000                                       # Weight for 'unkown' label
    ...
    with tf.Session() as sess:
        ...
        y_pred = convSpeechModelB(x_mel, x_mfcc, x_zcr, x_rmse,
        ...
        for i in xrange(num_iterations):
            ...
            if ((i + 1) % checkpoint_step == 0) or (i == num_iterations - 1):
                ...
                val_set_size = 6700
                for j in xrange(0, val_set_size, val_batch_size):
        ...
        total_conf_matrix = None
        test_set_size = 6700
        for j in xrange(0, test_set_size, test_batch_size):
            X_test, y_true_test = load_batch(df, cfg.DATA_DIR,
                                             batch_size=test_batch_size, silence_size=silence_size,
                                             label='test',
                                             random=False, seed=j,
                                             w=1.0, samples=cfg.SAMRATE)
